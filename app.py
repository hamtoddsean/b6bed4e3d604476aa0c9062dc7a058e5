                    ...
                    if plist['ww']!=[] and plist['status']!='sell':
                        if ask<= plist['ww'][0]:
                            if ask- plist['ww'][1]>=25:
                                kplacebuy=self.buy(float(plist['qty']),ask)
...
#####buy
    ...
    ###place buyorder
                                placebuy=self.buy(float(plist['qty']),ask)
...
if __name__ == "__main__":
    ...
    clist={'keeprecord':[],'go':'','qty':'0.011','pleven':'','ggo':'yes','list':[],'buyp':[],'sellp':[],'BUY':[],'SELL':[],'even':'','count':[],'status':'','buy':[],'sell':[],'lower':[],'lowert':[],'uppert':[],'upper':[],'w':[],'ww':[],'counter':[],'store':[],'tab':['d'],'watch':''}
    url='http://api.hitbtc.com/api/2/public/ticker/ETHUSD'
    stoptime=80000


    for vii in range(86400):
        time.sleep(1)
